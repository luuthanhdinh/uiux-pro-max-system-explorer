...
export function SystemCard({ design }: { design: SystemDesign }) {
    ...
    >
      ...
      <div className="relative h-36 overflow-hidden" style={{ background: design.colors.background }}>
        <div
          ...
          style={{ background: `linear-gradient(135deg, ${design.colors.primary}90 0%, ${design.colors.secondary}60 50%, ${design.colors.cta}40 100%)` }}
        />
        ...
        <div className="absolute top-3 right-3">
          <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${
            design.mood === 'dark' ? 'bg-slate-900/80 text-white/80' : 'bg-white/85 text-slate-900'
          }`}>
            {design.mood}
          </span>
        </div>
